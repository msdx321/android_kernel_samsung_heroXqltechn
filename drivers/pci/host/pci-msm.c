...
static void pcie_phy_init(struct msm_pcie_dev_t *dev)
{
	...
	msm_pcie_write_reg(dev->phy, QSERDES_COM_CLK_SELECT, 0x35);
	...
	msm_pcie_write_reg(dev->phy, QSERDES_COM_SSC_EN_CENTER, 0x00);
...
#ifdef CONFIG_SEC_BSP_PCIE_LINKFAIL_PANIC
		if(dev->linkdown_counter)
			panic("PCIe link initialization failed after linkdown");
#endif
		PCIE_INFO(dev, "PCIe: Assert the reset of endpoint of RC%d.\n",
			dev->rc_idx);
		gpio_set_value(dev->gpio[MSM_PCIE_GPIO_PERST].num,
...
int __init pcie_init(void)
{
	...
	//msm_pcie_debug_mask = 1; //jymask.park
