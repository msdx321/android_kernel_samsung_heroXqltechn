...
static uint32_t ln_irq_err_masks[] = { 
0x3f, 
0x00, 
0xe0, 
0x07, 
...
#define LN_IRQ_ERR_MASK(idx) (ln_irq_err_masks[idx]) 
	...
	struct msm_camera_csiphy_params *csiphy_params)
{
	...
	for (i = 0, mask = 0x1; i < MAX_LANES; i++) {
		...
		if (clk_lane == 1) {
			...
			msm_camera_io_w(0xC0, csiphybase +
				csiphy_dev->ctrl_reg->csiphy_3ph_reg.
				mipi_csiphy_2ph_lnn_cfg1_addr + offset);
		} else {
			msm_camera_io_w(0xD7, csiphybase +
				csiphy_dev->ctrl_reg->csiphy_3ph_reg.
				mipi_csiphy_2ph_lnn_cfg1_addr + offset);
		}
