...
static int adpd_power_enable(struct adpd143_data *data, int en)
{
	...
	if (data->is_sub_pmic) {
		...
		regulator_vdd_1p8 = regulator_get(&data->client->dev, "adpd143_1p8");
		if (IS_ERR(regulator_vdd_1p8) || regulator_vdd_1p8 == NULL) {
			pr_err("%s: vdd_1p8 regulator_get fail\n", __func__);
			return -ENODEV;
		}

		regulator_led_3p3 = regulator_get(&data->client->dev, "adpd143_3p3");
		if (IS_ERR(regulator_led_3p3) || regulator_led_3p3 == NULL) {
			pr_err("%s: vdd_3p3 regulator_get fail\n", __func__);
			regulator_put(regulator_vdd_1p8);
			return -ENODEV;
...
static int adpd_parse_dt(struct adpd143_data *data, struct device *dev)
{
	...
	data->hrm_int = of_get_named_gpio_flags(this_node,
		"adpd143,irq_gpio", 0, &flags);
	...
	if (of_property_read_u32(this_node, "adpd143,use-sub-pmic",
		&data->is_sub_pmic))
		data->is_sub_pmic = 0;
...
void adpd143_pin_control(struct adpd143_data *data, bool pin_set)
{
	int status = 0;
	data->p->state = NULL;
	if (pin_set) {
