...
int mdev_continue_for_TA_USB(muic_data_t *pmuic, int mdev)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if (pdesc->ccic_evt_rid || pdesc->ccic_evt_pdchg)
			break;
...
static void mdev_show_status(muic_data_t *pmuic)
{
	...
	pr_info("%s: mdev:%d rid:%d pdchg:%d attached:%d legacy_dev:%d\n", __func__,
			pdesc->mdev, pdesc->ccic_evt_rid, pdesc->ccic_evt_pdchg,
