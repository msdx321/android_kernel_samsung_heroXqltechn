...
static struct dsi_panel_cmds *mdss_hbm_etc(struct mdss_dsi_ctrl_pdata *ctrl, int *level_key)
{
	...
	*level_key = PANEL_LEVE1_KEY;
	...
	/* ACL off - Gallery 255 level in LDU/CCB */
	if (vdd->weakness_hbm_comp == 2) {
		if ((vdd->color_weakness_mode && (vdd->ccb_bl_backup == 255)) ||
			(vdd->ldu_correction_state && (vdd->ldu_bl_backup == 255)))
				vdd->dtsi_data[ctrl->ndx].hbm_etc_tx_cmds[vdd->panel_revision].cmds[5].payload[1] = 0x00; /* ACL OFF */
	}
...
static int mdss_force_acl_on(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	if ((vdd->weakness_hbm_comp == 2) && (bl_level == 255))
		acl_on = 0;
