...
static void da9155_charger_initialize(struct da9155_charger_data *charger)
{
	...
	/* Juntion temmperature 145C */
	da9155_update_reg(charger->i2c, DA9155_CONTROL_E, DA9155_TJUNC_WARN_MASK, DA9155_TJUNC_WARN_MASK);
}
...
static int da9155_pinctrl_select(struct da9155_charger_data *charger, bool on) {
	...
	if (!IS_ERR_OR_NULL(pins_i2c_state)) {
		...
		if (ret) {
			dev_err(charger->dev, "%s: Failed to set da9155 i2c pin state.\n", __func__);
			return ret;
		}
	}
...
static int da9155_i2c_pinctrl_init(struct da9155_charger_data *charger) {
	int ret = 0;
	struct pinctrl *i2c_pinctrl;

	charger->i2c_pinctrl = devm_pinctrl_get(&charger->i2c->dev);
	if (IS_ERR_OR_NULL(charger->i2c_pinctrl)) {
		dev_err(charger->dev, "%s: Failed to alloc mem for da9155 i2c pinctrl.\n", __func__);
	...
	charger->i2c_gpio_state_active = pinctrl_lookup_state(i2c_pinctrl, "da9155_active");
	if (IS_ERR_OR_NULL(charger->i2c_gpio_state_active)) {
		dev_err(charger->dev, "%s: Failed to set active state for da9155 i2c\n", __func__);
		ret = PTR_ERR(charger->i2c_gpio_state_active);
		goto err_i2c_active_state;
	}

	charger->i2c_gpio_state_suspend = pinctrl_lookup_state(i2c_pinctrl, "da9155_suspend");
	if (IS_ERR_OR_NULL(charger->i2c_gpio_state_suspend)) {
		dev_err(charger->dev, "%s: Failed to set suspend state for da9155 i2c\n", __func__);
		ret = PTR_ERR(charger->i2c_gpio_state_suspend);
		goto err_i2c_suspend_state;
	}

#if defined(CONFIG_OF)
	da9155_pinctrl_select(charger, true);
#endif
	return ret;
	...
	const struct i2c_device_id *id)
{
	...
	if (da9155_i2c_pinctrl_init(charger) < 0)
