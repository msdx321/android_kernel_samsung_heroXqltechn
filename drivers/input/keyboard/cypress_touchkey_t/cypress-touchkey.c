...
static int cypress_power_onoff(struct device *dev, bool onoff)
{
	...
	if (onoff) {
		if (!info->init_done || !regulator_is_enabled(info->touch_3p3)) {
			...
			dev_info(&info->client->dev, "%s: 3p3 is already enabled\n", __func__);
		}
		...
		if (!info->init_done || !regulator_is_enabled(info->vcc_en)) {
			...
			dev_info(&info->client->dev, "%s: vcc is already enabled\n", __func__);
		}
#ifdef USE_INT_PIN_PM_GPIO
		...
		if (regulator_is_enabled(info->touch_3p3)) {
			...
			dev_info(&info->client->dev, "%s: 3p3 is already disabled\n", __func__);
		...
		if (regulator_is_enabled(info->vcc_en)) {
			...
			dev_info(&info->client->dev, "%s: vcc is already disabled\n", __func__);

		}
	}

	dev_info(&info->client->dev, "%s: touch_3p3:%d, vcc_en:%d\n", __func__,
		regulator_is_enabled(info->touch_3p3),
		regulator_is_enabled(info->vcc_en));
	return 1;
}
...
static int load_fw_built_in(struct cypress_touchkey_info *info)
{
	...
	fw_name = kasprintf(GFP_KERNEL, "%s/%s.fw",
			TKEY_FW_BUILTIN_PATH, info->pdata->fw_name);
...
static int load_fw_in_sdcard(struct cypress_touchkey_info *info)
{
	...
	fw_name = kasprintf(GFP_KERNEL, "%s/%s.bin",
			TKEY_FW_IN_SDCARD_PATH, info->pdata->fw_name);
			...
			struct touchkey_device_tree_data *pdata)
{
	...
	rc = of_property_read_string(np, "cypress,fw_name", &pdata->fw_name);
	if (rc < 0) {
		dev_info(dev, "%s: Unable to get cypress,fw_name\n", __func__);
		return rc;
	}

	dev_err(dev,
			"%s: tkey_scl= %d, tkey_sda= %d, tkey_int= %d, sub_det= %d, boot= %d, fw_name= %s\n",
			__func__, pdata->gpio_scl, pdata->gpio_sda, pdata->gpio_int,
			pdata->gpio_sub_det, pdata->regulator_boot_on, pdata->fw_name);
