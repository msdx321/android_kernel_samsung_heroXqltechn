...
static void __ref
hotplug_work_fn(struct work_struct * work)
{
    ...
    for_each_online_cpu(cpu){
        ...
        #endif /* ifdef CONFIG_ALUCARD_HOTPLUG_USE_CPU_UTIL */
              ...
              rq_avg > pcpu_info->up_rq)
            {
                ...
                if (check_up) {
                    ...
                    pr_info("CPU[%u], UPCPU[%u], cur_freq[%u], cur_load[%u], rq_avg[%u], up_rate[%u]\n", cpu, upcpu,
                      cur_freq, cur_load, rq_avg, pcpu_info->cur_up_rate);
                    #endif
                    hotplug_onoff[upcpu]     = ON;
                    pcpu_info->cur_up_rate   = 1;
                    pcpu_info->cur_down_rate = 1;
...
static void __ref
alucard_hotplug_suspend(struct power_suspend * handler)
# else
static void __ref
alucard_hotplug_early_suspend(struct early_suspend * handler)
# endif
{
...
# ifdef CONFIG_POWERSUSPEND
static void __ref
alucard_hotplug_resume(struct power_suspend * handler)
# else
static void __ref
alucard_hotplug_late_resume(
...
static ssize_t
store_hotplug_sampling_rate(struct kobject * a,
  struct attribute * b,
  const char * buf, size_t count)
{
    int input;
...
/* hotplug_enable */
static ssize_t
store_hotplug_enable(struct kobject * a, struct attribute * b,
  const char * buf, size_t count)
{
    int input;
    int ret;
...
/* min_cpus_online */
static ssize_t
store_min_cpus_online(struct kobject * a, struct attribute * b,
  const char * buf, size_t count)
{
    int input;
    int ret;
...
/* maxcoreslimit */
static ssize_t
store_maxcoreslimit(struct kobject * a, struct attribute * b,
  const char * buf, size_t count)
{
    int input;
    int ret;
...
/* maxcoreslimit_sleep */
static ssize_t
store_maxcoreslimit_sleep(struct kobject * a,
  struct attribute * b,
  const char * buf, size_t count)
{
    int input;
...
/* hp_io_is_busy */
static ssize_t
store_hp_io_is_busy(struct kobject * a, struct attribute * b,
  const char * buf, size_t count)
{
 ...
 */
    ...
    defined(CONFIG_HAS_EARLYSUSPEND)
static ssize_t
store_hotplug_suspend(struct kobject * a,
  struct attribute * b,
  const char * buf, size_t count)
{
    int input;
...
#endif /* if defined(CONFIG_POWERSUSPEND) || defined(CONFIG_HAS_EARLYSUSPEND) */
...
alucard_hotplug_init(void)
{
    ...
    return ret;
} /* alucard_hotplug_init */

static void __exit
alucard_hotplug_exit(void)
{
    if (hotplug_tuners_ins.hotplug_enable > 0) {
        hotplug_stop();
    }
